...
#define XML_FILE "detectionAlgorithm.cid"
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " <da_id> <ip> <gateway>" << std::endl;
        return 1;
    }

    int da_id = atoi(argv[1]);
    ...
    char ied_name[100] = "";
    snprintf(ied_name, 100, "DA%02d", da_id);

    char mac_address[18] = "";
    snprintf(mac_address, 18, "01-0C-CD-01-00-%02d", da_id);

    update_ied_name(doc, ied_name);
    update_ied_ip(doc, ip, gateway);
    update_ied_mac(doc, mac_address);
    update_ied_appId(doc, da_id);

    update_sMonitoringVar(doc, ied_name, "MV_Line");
    update_sMonitoringVar(doc, ied_name, "MV_LineST");
